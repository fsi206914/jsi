...
public class RTree< Coord extends Comparable<? super Coord>> implements  Serializable {
  ...
  private Node[] qPickSeeds(LinkedList<Node> Nodes )
  {
    ...
    //GenericPoint<Double> s1 = new GenericPoint(Double.class, 2);
    //s1.setCoord(0, 0.0);s1.setCoord(1, 2.0);


    //GenericPoint<Double> s2 = new GenericPoint(Double.class, 2);
    //s2.setCoord(0, 2.0);s2.setCoord(1, 4.5);

	//Rectangle S = new Rectangle(Double.class, 2);
	//S.set(s1,s2);

	//System.out.println("begin displaying searching result");
    //List<Rectangle <Double>  > results = rtree.search(S);
