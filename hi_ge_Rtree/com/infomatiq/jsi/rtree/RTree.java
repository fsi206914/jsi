...
public class RTree implements SpatialIndex, Serializable {
  ...
  private void add(float minX, float minY, float maxX, float maxY, int id, int level) {
    ...
    Node newLeaf = null;
    log.info("in add------the id returned by choosenode is = " + n.nodeId );
  ...
  private Node splitNode(Node n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId) {
    ...
    log.info("in splitNode-------- "+newNode.nodeId + " is the node ID needed to be newed.");
  ...
  private void pickSeeds(Node n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId, Node newNode) {
    ...
    log.info( "pickSeeds---- mbrLenX = "+mbrLenX);
    log.info( "pickSeeds---- mbrLenY = "+mbrLenY);
    ...
    log.info( "in pickseeds----  tempHighestLow = "+tempHighestLow+ "  tempHighestLowIndex ="+tempHighestLowIndex);
    ...
    log.info( "in pickseeds----  highestLowIndex = "+highestLowIndex+ "  lowestHighIndex ="+lowestHighIndex);
    ...
    log.info( "in pickseeds --computing y----   n.entriesMaxY[i] = "+ n.entriesMaxY[i]);
    ...
    log.info( "in pickseeds --computing y----  tempHighestLow = "+tempHighestLow+ "  tempLowestHigh ="+tempLowestHigh);
    ...
    log.info( "in pickseeds --computing y----  highestLowIndex = "+highestLowIndex+ "  lowestHighIndex ="+lowestHighIndex);
  ...
  private Node chooseNode(float minX, float minY, float maxX, float maxY, int level) {
    ...
    while (true) {
      ...
      log.info(" in chooseNode n.level = " + n.level);
      ...
      log.info(" leasttempMinX = " + n.entriesMinX[0] + " leasttempMinY = " + n.entriesMinY[0]);
      log.info(" leasttempmaxX = " + n.entriesMaxX[0] + " leasttempmaxY = " + n.entriesMaxY[0]);
      ...
      log.info(" leastEnlargement = " + leastEnlargement);
  ...
  private Node adjustTree(Node n, Node nn) {
    ...
    log.info( " n.level = "+n.level + "   treeHeight = "+treeHeight);
    while (n.level != treeHeight) {
      ...
      if (nn != null) {
        if (parent.entryCount < maxNodeEntries) {
          ...
          log.info( " in adjustTree---- adjust Tree here. ");
